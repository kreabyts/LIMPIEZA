...
function loadProducts() {
    const stored = localStorage.getItem('kreabyts_products_v3');
    if (stored) {
        try {
...
function saveProducts() {
    localStorage.setItem('kreabyts_products_v3', JSON.stringify(products));
}
    ...
    grid.innerHTML = items.map(p => {
        ...
        return `
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-xl hover:border-indigo-300 transition duration-300 flex flex-col justify-between group relative overflow-hidden">
                ...
                <!-- Imagen del producto (grande arriba) o ícono si no hay imagen -->
                ${p.image
                    ? `<div class="w-full h-32 sm:h-40 overflow-hidden bg-slate-100">
                           <img src="${p.image}" alt="${p.name}" class="w-full h-full object-cover" onerror="this.parentElement.innerHTML='<div class=\\'w-full h-full flex items-center justify-center bg-indigo-50 text-indigo-400 text-4xl\\'><i class=\\'fa-solid ${p.icon || 'fa-bottle-droplet'}\\'></i></div>'">
                       </div>`
                    : `<div class="w-full h-20 sm:h-24 bg-indigo-50 flex items-center justify-center text-indigo-400 text-4xl sm:text-5xl group-hover:bg-indigo-600 group-hover:text-white transition duration-300">
                ...
                <div class="pt-2 sm:pt-3 border-t border-slate-100 space-y-2">
                    ...
                    <button onclick="addToCart(${p.id})" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2 sm:py-2.5 px-2 rounded-xl text-[11px] sm:text-sm font-bold shadow-md shadow-indigo-100 transition active:scale-95 flex items-center justify-center gap-1.5">
